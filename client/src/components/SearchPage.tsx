...
const SearchPage = () => {
  ...
  // When URL param or applied filters change, run search.
  useEffect(() => {
    const searchText = params.text || "";
    searchRestaurant(searchText, searchQuery, appliedFilter);
  }, [params.text, appliedFilter]);

  // Clear any persisted filters on initial mount so the UI starts without unintended filters
  useEffect(() => {
    resetAppliedFilter();
  }, []);
  ...
  const handleSearch = () => {
    ...
    // update the URL so it reflects the current search and triggers useEffect
    navigate(`/search/${encodeURIComponent(term)}`);
    // also call immediately to avoid waiting for URL param propagation
    searchRestaurant(term, term, appliedFilter);
  };
                  ...
                  >
                    ...
                    <CardFooter className="p-4 border-t dark:border-t-gray-700 border-t-gray-100 text-white flex justify-end">
                      <Link to={`/restaurant/${restaurant._id}`}>
                        <Button className="bg-[var(--orange)] hover:bg-[var(--hoverOrange)] font-semibold py-2 px-4 rounded-full shadow-md transition-colors duration-200">
                          View Menus
                        </Button>
                      </Link>
                    </CardFooter>
