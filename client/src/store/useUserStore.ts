...
type UserState = {
    ...
    loading: boolean;
    signup: (input: SignupInputState) => Promise<any>;
...
export const useUserStore = create<UserState>()(persist((set) => ({
    ...
    updateProfile: async (input: any) => {
        try {
            // server route expects POST on /profile/update
    ...
    toggleAdmin: async () => {
        try {
            set({ loading: true });
            const response = await axios.post(`${API_END_POINT}/profile/toggle-admin`);
            if (response.data.success) {
                toast.success(response.data.message);
    ...
    {
        name: 'snapbite-user', // Changed name to reset old stuck state
